[**legal-doc-compiler**](../README.md)

***

[legal-doc-compiler](../modules.md) / inputSchema

# inputSchema

Input schema definitions for legal document compilation.
Defines Zod validators and types for document structure, metadata, and content.

## Sort Strategy

source-order

## Type Aliases

- [EntryType](type-aliases/EntryType.md)
- [EntryID](type-aliases/EntryID.md)
- [AuthorRole](type-aliases/AuthorRole.md)
- [Author](type-aliases/Author.md)
- [DocumentProperties](type-aliases/DocumentProperties.md)
- [Version](type-aliases/Version.md)
- [DocumentFeatures](type-aliases/DocumentFeatures.md)
- [Options](type-aliases/Options.md)
- [Globals](type-aliases/Globals.md)
- [ContentHeading](type-aliases/ContentHeading.md)
- [ContentParagraph](type-aliases/ContentParagraph.md)
- [Content](type-aliases/Content.md)
- [Document](type-aliases/Document.md)
- [Input](type-aliases/Input.md)

## Variables

- [EntryType](variables/EntryType.md)
- [EntryID](variables/EntryID.md)
- [AuthorRole](variables/AuthorRole.md)
- [Author](variables/Author.md)
- [DocumentProperties](variables/DocumentProperties.md)
- [Version](variables/Version.md)
- [DocumentFeatures](variables/DocumentFeatures.md)
- [DocumentOptions](variables/DocumentOptions.md)
- [Globals](variables/Globals.md)
- [ContentHeading](variables/ContentHeading.md)
- [ContentParagraph](variables/ContentParagraph.md)
- [Content](variables/Content.md)
- [Document](variables/Document.md)
- [Input](variables/Input.md)
